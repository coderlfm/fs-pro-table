...
export default memo((props: propsType) => {
    ...
    return (
        <Form layout="inline" form={form} name="control-hooks" onFinish={onFinish} {...formProps?.layoutConfig} className={`pro-table-form-wrap ${circle ? 'pro-table-form-circle' : ''}`}>
                ...
                <Space>
                    ...
                    </Button>
                    {
                        formProps?.config?.otherBtn?.map(item => {
                            return (<Button {...item.btnProps} onClick={()=>{
                                const FormFields = form.getFieldsValue();
                                item.onBtnChange(FormFields)
                            }} className="btn-primary" key={item}>
                                {item.text}
                            </Button>)
                        })
                    }
